 ...
 * calculate delta
 */
public class CalcUtilGp extends CalcUtil {
    ...
    public CalcUtilGp(StockVo stock) throws MidasException {
        init(stock);
    }
    ...
    public boolean isTrendUpWeak(int index){
        return (pMa10D1[index] > 0) && (pMa20D1[index] > 0) && (pMa30D1[index] > 0) && (pMa60D1[index] > 0);
    }
    ...
    public boolean isVolumeDownAnormal(int index){
        if( total[index] / total[index - 1] < 0.4 ){
            return true;
        }
        return false;
    }
