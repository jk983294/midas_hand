 ...
 * use weight matrix
 */
public class GraphLinkMatrix {

    private static final Logger logger = Logger.getLogger(GraphLinkMatrix.class);
    ...
    public GraphType type = GraphType.UNDIRECTED;

    public GraphLinkMatrix() {
    }

    public GraphLinkMatrix(GraphType type) {
        this();
        this.type = type;
    }

    public GraphLinkMatrix(GraphLinkMatrix g) {
        // Deep copies
        this.type = g.type;
        this.n = g.n;
        weight = new int[n][n];
    ...
    public GraphLinkMatrix(int n) {
        this(GraphType.UNDIRECTED, n);
    }

    public GraphLinkMatrix(GraphType type, int n) {
        this.type = type;
        this.n = n;
        weight = new int[n][n];
        for(int i =0; i < n; i++){
    ...
    public void setUnDirectedEdge(int i,int j,int len){
        weight[i][j] = weight[j][i] = len;
        m++;
    }
